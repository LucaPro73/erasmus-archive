...
let mainMenuLoopBuffer, node, mainMenuAudioElement
...
function playMainMenuLoop() {
    if (!mainMenuLoopBuffer) {
        if (!mainMenuAudioElement) {
            mainMenuAudioElement = new Audio("./assets/menuloop.mp3")
            mainMenuAudioElement.loop = true
            mainMenuAudioElement.load()
        }
        mainMenuAudioElement.currentTime = 0
        mainMenuAudioElement.play()
    }
    else {
        if (node) node.stop()
...
function stopMainMenuLoop() {
    if (mainMenuAudioElement) {
        mainMenuAudioElement.pause()
    }
    else node.stop()
}
...
let currentScore = 0
let currentStyle = 1
let lastStyleGainTime = 0
const maxStyle = 5.15
const stylePerDash = 0.55
const styleDecayRate = 0.2
const styleDecayStartDelay = 1250
...
try {
    levels.unshift(level_AuroraBorealis)
} catch (e) {
    console.error(e)
}

function getHighScoreKey(level) {
    return `highScore_${level.name.replace(/\s/g, '')}`
}
const highScores = Object.fromEntries(Object.entries(levels).map(l => {
    if ("stage" in l[1]) return [l[1].name, parseInt(localStorage.getItem(getHighScoreKey(l[1])) || 0)]
}).filter(i => i))

let levelCompleteData = {
    name: undefined,
    score: 0,
    dashCount: 0,
    rank: "D"
...
function triggerScreenShake() {
    let offx = 0, offy = 0
    let i = setInterval(() => {
        let dx = (Math.random() - 0.5) * 2 * shakeStrength
        let dy = (Math.random() - 0.5) * 2 * shakeStrength
        offx += dx
        offy += dy
        ctx.translate(dx, dy)
    }, 40)
...
function playSelectedAudio(seek = 0) {
    ...
    if (!audio.buffer) {
        audio.fallbackAudioElement.load()
        audio.fallbackAudioElement.currentTime = seek
        audio.fallbackAudioElement.play()
        return
    }
...
function stopSelectedAudio() {
    ...
    if (audio.buffer && audio.source) {
        ...
        audio.fallbackAudioElement.pause()
    }
    audio.playing = false
}
function setSelectedAudioVolume(vol) {
    selectedLevel.audio.gainNode.gain.value = vol
    selectedLevel.audio.fallbackAudioElement.volume = vol
}
...
function clearLevel() {
    ...
    selectedLevel.audio.fallbackAudioElement.currentTime = 0
    obstacleState = []
    player = { ...initPlayer }
    currentScore = 0
    currentStyle = 1.0
...
function songUpdate(time = 0) {
    ...
    if (!startTime) {
        ...
        startTime = time
        dashCount += parseInt(localStorage.getItem("dashCount") || "0")
        lastStyleGainTime = time
    ...
    if (beat <= easeDuration)
        setSelectedAudioVolume(Math.max(beat / easeDuration, 0))

    if (currentStyle > 1.0 && (time - lastStyleGainTime) > styleDecayStartDelay) {
        currentStyle = Math.max(1.0, currentStyle - styleDecayRate * (deltaTime / 1000))
    }
    if (player.hp > 0 && !levelEnd) {
        currentScore += pointsPerSecond * (1 + Math.floor(currentStyle) * 0.6) * (deltaTime / 1000)
    }

    ctx.clearRect(0, 0, width, height)
    drawBackground(-20, bgScrollSpeed, deltaTime)

    let tutorialOpacity1 = Math.min(Math.max(1 - (beat - 7) / 2, 0), 1)
    ctx.fillStyle = `rgba(64, 64, 64, ${tutorialOpacity1})`
    ctx.font = "20px monospace"
    ctx.textAlign = "right"
    ctx.fillText("Your LIVES are up here ^", width - 62, 75)
    ...
    let tutorialOpacity2 = 0
    if (beat >= 7.5 && beat < 9.5) {
        tutorialOpacity2 = (beat - 7.5) / 2
    } else if (beat >= 9.5 && beat < 14) {
        tutorialOpacity2 = 1
    } else if (beat >= 14 && beat < 17) {
        tutorialOpacity2 = 1 - ((beat - 14) / 2)
    } else {
        tutorialOpacity2 = 0
    }
    ctx.fillStyle = `rgba(64, 64, 64, ${tutorialOpacity2})`
    ctx.fillText("DASH through obstacles to gain STYLE", width / 2, height / 3)
    ctx.fillText("STYLE increases your overall SCORE", width / 2, height / 3 * 2)
        ...
        if (beat - launchBeat >= -obstacle.windup && beat - launchBeat <= obstacle.lifespan) {
            ...
            for (let i = 0; i < obstacle.repeatCount; i++) {
                ...
                if (canCollide && wouldCollide(scaleRect(player, 0.9), obj)) {
                    if (player.invUntil >= time) {
                        if (player.dashSpeed > 4 && lastStyleGainTime + styleDecayStartDelay / 2 < time) {
                            currentStyle = Math.min(maxStyle, currentStyle + stylePerDash)
                            lastStyleGainTime = time
                        }
                    } else {
                        hit.load()
    ...
    let rankIndex = Math.floor(currentStyle - 1)
    rankIndex = Math.min(rankIndex, ranks.length - 1)
    let currentRank = ranks[rankIndex]

    let progressToNextRank = 0
    if (rankIndex < ranks.length - 1) {
        progressToNextRank = (currentStyle - (rankIndex + 1))
        progressToNextRank = Math.min(1, Math.max(0, progressToNextRank))
    } else {
        progressToNextRank = 1
    }
    const scoreDisplayText = `SCORE: ${Math.round(currentScore)} | STYLE: ${currentRank}`
    const textSize = ctx.measureText(scoreDisplayText).width
    const barX = width / 2 + textSize / 2

    ctx.font = "28px monospace"
    ctx.textAlign = "center"
    ctx.fillStyle = "#404040"
    ctx.globalAlpha = beat > 12 ? 1 : tutorialOpacity2
    ctx.fillText(scoreDisplayText, width / 2 - textSize / 2, height - 40)

    ctx.fillRect(barX, height - 46, 120, 12)

    ctx.fillStyle = "#808080"
    ctx.fillRect(barX, height - 46, 120 * progressToNextRank, 12)
    ctx.globalAlpha = 1

    if (levelEnd && !transitionFrom && !inTransitionEnd) {
        transitionFrom = time
        drawTransitionStart("LEVEL CLEAR", null, transitionFrom)

        levelCompleteData.score = Math.round(currentScore)
        levelCompleteData.dashCount = player.dashCount

        const thresholds = selectedLevel.scoreThresholds
        if (levelCompleteData.score >= thresholds.s) levelCompleteData.rank = "S"
        else if (levelCompleteData.score >= thresholds.a) levelCompleteData.rank = "A"
        else if (levelCompleteData.score >= thresholds.b) levelCompleteData.rank = "B"
        else if (levelCompleteData.score >= thresholds.c) levelCompleteData.rank = "C"
        else levelCompleteData.rank = "D"

        const levelHighScoreKey = getHighScoreKey(selectedLevel)
        if (levelCompleteData.score > highScores[selectedLevel.name]) {
            localStorage.setItem(levelHighScoreKey, levelCompleteData.score.toString())
            highScores[selectedLevel.name] = levelCompleteData.score
        }
        levelCompleteData.name = selectedLevel.name

        claimAchievement("And So It Beings")
        if (player.hp === 3) claimAchievement("Not Even A Scratch")
        updateAchievements()
        localStorage.setItem("dashCount", dashCount)
    } else if (transitionFrom && !inTransitionEnd) {
        const fade = Math.max((transitionFrom - time + 1500) / 1500, 0)
        setSelectedAudioVolume(Math.min(selectedLevel.audio.gainNode.gain.value, fade))
        if (drawTransitionStart("LEVEL CLEAR", null, transitionFrom)) {
            clearLevel()
            transitionEndStart = time
            inTransitionEnd = true
            return requestID = requestAnimationFrame(levelCompleteUpdate)
        }
    }
...
function claimAchievement(name) {
    ...
    if (ach && !ach.obtained) {
        ...
        console.error(`Couldn't find achievement "${name}"`)
    }
}
...
function mainMenuUpdate(time = 0) {
    ...
    if (inTransitionEnd) {
        if (drawTransitionEnd("MAIN MENU", null, transitionEndStart)) {
            inTransitionEnd = false
            transitionFrom = 0
            mouse1click = false
        }
...
function levelSelectUpdate(time) {
    ...
    levels.forEach((l, i) => {
        ...
        if (l.name in highScores) {
            ctx.font = "24px monospace"
            ctx.fillStyle = "#808080"
            ctx.textAlign = "right"
            ctx.fillText(`High score: ${highScores[l.name]}`, width - 50, height / 3 + i * 120 - 50)
        }
        ctx.fill()
    })

    if (levels.length === 1) {
        ctx.fillText("(failed to load levels, check the console)", width / 2 - 100, height / 2)
    }
...
function levelCompleteUpdate(time = 0) {
    const deltaTime = time - lastTime
    lastTime = time

    ctx.clearRect(0, 0, width, height)
    drawBackground(75, 0.5, deltaTime)
    ...
    ctx.font = "40px monospace"
    ctx.fillStyle = "#808080"
    ctx.fillText("LEVEL CLEAR", width / 2, 55)
    ...
    ctx.fillStyle = levelCompleteData.rank === "S" ? "#ffff00" : "#808080"
    ctx.textAlign = "center"
    ctx.textBaseline = "middle"
    ctx.fillText(levelCompleteData.rank, circleX, circleY)

    const leftX = width * 0.25
    const scoreY = height / 2 - 75
    ctx.textAlign = "center"
    ctx.textBaseline = "top"

    ctx.font = "bold 60px monospace"
    ctx.fillStyle = "#808080"
    ctx.fillText(`Score: ${levelCompleteData.score}`, leftX, scoreY)
    ctx.font = "30px monospace"
    ctx.fillStyle = "#404040"
    ctx.fillText(`High score: ${highScores[levelCompleteData.name]}`, leftX, scoreY + 80)
    ctx.fillText(`Dashes used: ${levelCompleteData.dashCount}`, leftX, scoreY + 130)

    const buttonText = "[LEFT CLICK] to continue"
    const buttonY = height - 80
    const buttonFont = "36px monospace"
    ctx.textAlign = "center"
    ctx.textBaseline = "middle"
    ctx.font = buttonFont
    const pulse = 0.5 + 0.5 * Math.sin(time * 0.003)
    ctx.fillStyle = lerpColor("#404040", "#808080", pulse)
    ctx.fillText(buttonText, width / 2, buttonY)

    if (inTransitionEnd) {
        if (drawTransitionEnd("LEVEL CLEAR", null, transitionEndStart)) {
            inTransitionEnd = false
            mouse1click = false
        }
    } else if (mouse1click && !transitionFrom && !inTransitionEnd) {
        transitionFrom = time
        click.play()
    } else if (transitionFrom && !inTransitionEnd) {
        if (drawTransitionStart("MAIN MENU", null, transitionFrom)) {
            transitionEndStart = time
            inTransitionEnd = true
    ...
    requestID = requestAnimationFrame(levelCompleteUpdate)
}
...
document.addEventListener('keydown', e => {
    if (e.key.toLowerCase() === "k" && e.shiftKey) {
        ...
        Object.entries(highScores).forEach(([name, _]) => localStorage.removeItem(getHighScoreKey({ name })))
        location.reload()
    }
})    